# RNASeqEnrich.shinyapp
# A R/shiny tool to enrich filtered RNASeq statistical analysis results
# This tool outputs enrichment tables and plots from list of filtered genes (DE signature)
# such file can be produced with the related tool RNASeqFiltering
...
ui <- fluidPage(
  ...
  # Application header
  headerPanel(tags$h3("Enrich filtered RNASeq data (LogFC and corPvalues)")),

  # Application title
  titlePanel(
    windowTitle = "RNASeq Enrichment Tool",
...
server <- function(input, output) {
  ...
  output$downloadData <- downloadHandler(
    filename <- function() { paste("StatisticalResults_filtered-data", "xlsx", sep=".") },
    content <- function(file) { file.copy("Data/StatisticalResults_filtered-data.xlsx", file) }
  )
